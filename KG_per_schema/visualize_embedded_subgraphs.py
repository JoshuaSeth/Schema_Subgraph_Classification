'''Final check whether the subgraphs make sense, right before entering the NN'''
...
from subgraph_viz_example import visualize_some_subgraph
import torch
...
def build_graph_part(research_sents_or_not, mode, schema_name):
    ...
    for idx, rel_sent in enumerate(rels):
        if len(rel_sent) > 0:
            ...
            if idx == 0:
                print('Subgraphs corresponds to', data)
                print('edge_types_indices', edge_types_indices)
                print('edge_indices', edge_indices)
                print('node_features', node_features)
                visualize_some_subgraph(
                    schema_name, mode, False, research_sents_or_not, idx)
