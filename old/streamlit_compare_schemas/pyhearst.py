...
class PyHearst:
    ...
    def merge_consecutive_NPs(self, annotated_sentence: str) -> str:
        ...
        # inidices of all NP_-tagged tokens
        NP_indices = [i for i in range(len(toks)) if toks[i].startswith('NP_')]
        ...
        for i in range(len(NP_indices)-1, 0, -1):
            if NP_indices[i]-1 == NP_indices[i-1]:
                toks[NP_indices[i-1]:NP_indices[i] +
                     1] = [toks[NP_indices[i-1]] + '_' + toks[NP_indices[i]][3:]]
    ...
    def extract_patterns(self, sent: str) -> List:
        ...
        for pattern in self.patterns:
            # get each instance of the regex pattern
            matches = re.findall(pattern[0], annotated_sentence)
